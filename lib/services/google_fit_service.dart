...
class GoogleFitService {
  ...
  Future<int?> getDailySteps(DateTime date) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _logger.e('Failed to get steps data: ${response.statusCode} - ${response.body}');
      }
  ...
  Future<double?> getDailyCaloriesBurned(DateTime date) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _logger.e('Failed to get calories data: ${response.statusCode} - ${response.body}');
      }
  ...
  Future<double?> getDailyDistance(DateTime date) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
          ...
          if (dataset.isNotEmpty) {
            ...
            if (dataPoint.isNotEmpty) {
              ...
              if (value.isNotEmpty) {
                ...
                final meters = (value.first['fpVal'] as num?)?.toDouble() ?? 0.0;
        ...
        _logger.e('Failed to get distance data: ${response.statusCode} - ${response.body}');
      }
        ...
        if (dataSources != null) {
          ...
          for (final source in dataSources) {
            if (source['dataType']['name'] == 'com.google.weight') {
              ...
              final weightUrl = '$_baseUrl/users/me/dataSources/$sourceId/datasets/latest';
  ...
  Future<Map<String, dynamic>?> getFitnessData(DateTime startDate, DateTime endDate) async {
    if (!_isAuthenticated || _authClient == null) {
      _logger.w('Not authenticated with Google Fit');
      return null;
    }
