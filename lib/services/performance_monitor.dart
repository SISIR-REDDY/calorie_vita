...
class PerformanceMonitor {
  ...
  List<Map<String, dynamic>> getSlowOperations({int thresholdMs = 1000}) {
    ...
    slowOps.sort((a, b) => (b['average_ms'] as int).compareTo(a['average_ms'] as int));
    
    return slowOps;
  }

  /// Get recent events of a specific type
  List<Map<String, dynamic>> getRecentEvents(String eventType, {int limit = 10}) {
    final filteredEvents = _events.where((event) => event.eventType == eventType).toList();
    final startIndex = filteredEvents.length > limit ? filteredEvents.length - limit : 0;
  ...
  List<String> getPerformanceRecommendations() {
    ...
    for (final op in slowOps) {
      ...
      if (operation.contains('firebase')) {
        recommendations.add('Consider adding caching for $operation (avg: ${avgMs}ms)');
      } else if (operation.contains('image')) {
        recommendations.add('Consider image optimization for $operation (avg: ${avgMs}ms)');
      } else if (operation.contains('network')) {
        recommendations.add('Check network connectivity for $operation (avg: ${avgMs}ms)');
      } else {
